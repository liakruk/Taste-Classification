...
model = tf.keras.models.Sequential([
	...
	tf.keras.layers.BatchNormalization(),
	tf.keras.layers.Dropout(0.1),
	tf.keras.layers.Dense(2, activation='softmax', kernel_regularizer=L2(1e-4))
	])
...
history = model.fit(x_train,y_train,
					...
					callbacks=[
					tf.keras.callbacks.EarlyStopping(monitor='loss',patience=80,min_delta=0.001,restore_best_weights=True),
					reduce_on_plateau
				])
